...
<div class="main">
	<div class="container">
		<div class="photo">
			<img alt="" src="image/front1.jpg" width="500" height="350" class="img-rounded">
		</div>
		<form method="post" action="VerifyLoginServlet">
			...
			<div class="row">
				...
				function checkForm(){//再次使用ajax提交检测用户信息
					$.ajax({
						url : "RoleAction_Login.action",//todo:set the servelet of your own
						type : "post",
						data : {
						...
						success : function(data) {
							if(data.result=="User_NotFound"){//用户不存在
								...
								window.location.href="RoleLogin.action";
							}
						}
					});
				}
			...
			<div class="row">
				<input class="btn btn-success" type="button" onclick="geetest_ajax_results()" value="登录" id="submit-button" />
			</div>
		</form>
	</div><!-- end container -->
</div><!-- end main -->
